...
## 1. コンセプト
Rinkerは、エンジニアが使い慣れたターミナルの操作感をSlack上で再現する。対話だけでなく、フラグ（オプション）を用いた効率的なコマンド操作で一般的なLLMサービスとの差別化を図る。
...
| :--- | :--- | :--- | :--- |
| **`sum`** | `-w`, `--week` | 週間進捗要約の生成。企業提出用の下書きを作成。 | **Rinker3 Plus** |
| | `-d`, `--day` | 本日の日報要約の生成。 | **Rinker3 Sonic** |
| **`stat`** | `-v`, `--verbose` | TechLeader Scoreの指標別詳細（TES/PCS等）を表示。 | **Rinker3 Sonic** |
| | `-g`, `--graph` | 成長曲線のグラフ表示（Looker Studio連携）。 | **Rinker3 Sonic** |

### Scouting (企業連携)
| コマンド | オプション | 内容 | 駆動モデル |
| :--- | :--- | :--- | :--- |
| **`push`** | `-f`, `--force` | 現在の要約を企業ポータルへ即時送信（Shout）。 | **Rinker3 Plus** |
| | `--draft` | 企業送信用の下書きを保存・確認。 | **Rinker3 Plus** |

### Scheduling (RinkerBell)
| コマンド | オプション | 内容 | 駆動モデル |
| :--- | :--- | :--- | :--- |
| **`bell`** | `-ls`, `--list` | 予約可能な面談スロットを一覧表示。 | **Rinker3 Sonic** |
| | `-n`, `--now` | 「今すぐ面談可能」ステータスへの切り替え。 | **Rinker3 Sonic** |
...
* **Rinker3 Sonic (Gemini 3 Flash)**
    * **役割**: 低レイテンシ応答、簡易QA、スコア照会、日程確認。
* **Rinker3 Plus (Gemini 3 Pro)**
